...
bimixEM = function (array_lik, prior, pi_init = NULL, control = list()) 
{
    control.default = list(K = 1, method = 3, square = TRUE, 
        ...
        tol = 1e-07, maxiter = 50000, trace = FALSE)
    namc = names(control)
...
write(as.numeric(bioutput$converged), file = '../output/conv.moreiter.txt', append=TRUE)
write(LLRT, file = '../output/LLRT.moreiter.txt', append = TRUE)
write(w, file = '../output/log.moreiter.txt', append = TRUE)
write(as.vector(t)[3], file = '../output/itertime.moreiter.txt', append = TRUE)
}
