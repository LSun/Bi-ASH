...
plot(seq(-3.5, 3.5, by = 0.01), dnorm(seq(-3.5, 3.5, by = 0.01)), type = "l", lty = 3, lwd = 2, xlab = "signal", ylab = "density", main = expression("(a)"))
...
legend("topleft", pch = c(13, 20), col = c("blue", "black"), c(expression(paste('{', theta[1*i], '}')), expression(paste('{', alpha[2*j], '}'))), bty = "n")
text(x = c(0, 0), y = c(0.38, 0.15), c(expression("f"), expression("h")), col = c("black", "blue"))

plot(seq(-3.5, 3.5, by = 0.01), dnorm(seq(-3.5, 3.5, by = 0.01)), type = "l", lty = 3, lwd = 2, xlab = "signal", ylab = "density", main = expression("(b)"))
...
legend("topleft", pch = c(13, 1, 4), col = c("blue", "black", "red"), c(expression(paste('{', theta[1*i], '}')), expression(paste('{', alpha[1*i], '}')), expression(paste('{', beta[1*i], '}'))), bty = "n")
